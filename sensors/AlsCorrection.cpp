...
float AlsCorrection::process(const Event& event) {
    ...
    auto it_range = std::find_if(conf_.linearity_ranges.begin(), conf_.linearity_ranges.end(),
                                 [brightness](const BrightnessRange& range) {
                                     return brightness >= range.bright_min &&
                                            brightness <= range.bright_max;
                                 });
